#include <aarch64/intrinsic.h>
#include <driver/aux.h>
#include <driver/gpio.h>
#include <driver/uart.h>

void init_uart() {
    device_put_uint32(GPPUD, 0);
    delay_us(5);
    device_put_uint32(GPPUDCLK0, (1 << 14) | (1 << 15));
    delay_us(5);
    device_put_uint32(GPPUDCLK0, 0);

    // enable mini uart and access to its registers.
    device_put_uint32(AUX_ENABLES, 1);
    // disable auto flow control, receiver and transmitter (for now).
    device_put_uint32(AUX_MU_CNTL_REG, 0);
    // enable receiving interrupts.
    device_put_uint32(AUX_MU_IER_REG, 3 << 2 | 1);
    // enable 8-bit mode.
    device_put_uint32(AUX_MU_LCR_REG, 3);
    // set RTS line to always high.
    device_put_uint32(AUX_MU_MCR_REG, 0);
    // set baud rate to 115200.
    device_put_uint32(AUX_MU_BAUD_REG, AUX_MU_BAUD(115200));
    // clear receive and transmit FIFO.
    device_put_uint32(AUX_MU_IIR_REG, 6);
    // finally, enable receiver and transmitter.
    device_put_uint32(AUX_MU_CNTL_REG, 3);
}

uint8_t uart_get_char() {
    int state = device_get_uint32(AUX_MU_IIR_REG);
    if ((state & 1) || (state & 6) != 4)
        return 0xff;

    return device_get_uint32(AUX_MU_IO_REG) & 0xff;
}

void uart_put_char(uint8_t c) {
    while (!(device_get_uint32(AUX_MU_LSR_REG) & 0x20)) {}

    device_put_uint32(AUX_MU_IO_REG, c);

    // fix Windows's '\r'.
    if (c == '\n')
        uart_put_char('\r');
}
